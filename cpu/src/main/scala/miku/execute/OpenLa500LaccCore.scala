...
final case class OpenLa500LaccRequest() extends Bundle {
  val command = Bits(OpenLa500LaccCommand.Width bits)
  val immediate = Bits(7 bits)
  val registerJ = Bits(32 bits)
  val registerK = Bits(32 bits)
}

final case class OpenLa500LaccResponse() extends Bundle {
  val data = Bits(32 bits)
}

/** One data-cache transaction issued by the accelerator. */
final case class OpenLa500LaccMemoryRequest() extends Bundle {
...
final case class OpenLa500LaccMemoryResponse() extends Bundle {
  val data = Bits(32 bits)
}
...
/** Cycle-compatible implementation of `a158aa8:rtl/lacc_core.v` and `rtl/lacc_demo.v`.
  ...
  */
final class OpenLa500LaccCore extends Component {
  val io = new Bundle {
    val clk = in Bool ()
    val reset = in Bool ()
    val flush = in Bool ()
    val request = slave(Flow(OpenLa500LaccRequest()))
    val response = master(Flow(OpenLa500LaccResponse()))
    val memoryRequest = master(Stream(OpenLa500LaccMemoryRequest()))
    val memoryResponse = slave(Flow(OpenLa500LaccMemoryResponse()))
  }
  ...
  private val logic = new ClockingArea(laccClockDomain) {
    ...
    val operationLmadd = io.request.payload.command === OpenLa500LaccCommand.Lmadd
    val operationConfigure = io.request.payload.command === OpenLa500LaccCommand.Configure
...
private final class LegacyOpenLa500LaccCore extends Component {
  ...
  val io = new Bundle {
    ...
    val lacc_req_command = in Bits (OpenLa500LaccCommand.Width bits)
...
object GenerateOpenLa500LaccCore {
    ...
    args match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "usage: GenerateOpenLa500LaccCore [--out-dir] <output-directory>"
        )
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    config.generateVerilog(new LegacyOpenLa500LaccCore)
  }
}
