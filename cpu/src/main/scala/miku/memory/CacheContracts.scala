...
final case class CacheMaintenanceRequest(config: OooCoreConfig) extends Bundle {
...
final case class CacheMaintenanceResponse(config: OooCoreConfig) extends Bundle {
  val robPointer = UInt(config.robPointerWidth bits)
  val recoveryEpoch = UInt(config.recoveryEpochWidth bits)
}

final case class CacheRequest(config: OooCoreConfig) extends Bundle {
  val virtualAddress = UInt(config.xlen bits)
  val physicalAddress = UInt(config.xlen bits)
  val isWrite = Bool()
...
final case class CacheResponse(config: OooCoreConfig) extends Bundle {
  val robPointer = UInt(config.robPointerWidth bits)
  val recoveryEpoch = UInt(config.recoveryEpochWidth bits)
  val pdst = UInt(config.physicalRegIndexWidth bits)
  val loadQueueIndex = UInt(config.loadQueueIndexWidth bits)
...
/** One aligned frontend fetch group.
  ...
  */
final case class InstructionCacheRequest(config: OooCoreConfig) extends Bundle {
  val virtualAddress = UInt(config.xlen bits)
  val physicalAddress = UInt(config.xlen bits)
  val uncached = Bool()
}

final case class InstructionCacheResponse(config: OooCoreConfig) extends Bundle {
  val virtualAddress = UInt(config.xlen bits)
  val physicalAddress = UInt(config.xlen bits)
  val instructions = Vec(Bits(32 bits), config.fetchWidth)
  val predecode = Vec(FetchPredecode(config), config.fetchWidth)
  val error = Bool()
}

final case class LineReadRequest(config: OooCoreConfig) extends Bundle {
...
final case class LineReadBeat(config: OooCoreConfig) extends Bundle {
  val mshrId = UInt(log2Up(config.mshrEntries) bits)
  val beat = UInt(CacheContract.BeatIndexWidth bits)
  val data = Bits(CacheContract.BeatBits bits)
  val last = Bool()
  val error = Bool()
}

final case class LineWriteRequest(config: OooCoreConfig) extends Bundle {
  val lineAddress = UInt(config.xlen bits)
  val data = Bits(CacheContract.LineBits bits)
  val byteMask = Bits(CacheContract.LineBytes bits)
  val mshrId = UInt(log2Up(config.mshrEntries) bits)
}

final case class LineWriteResponse(config: OooCoreConfig) extends Bundle {
  val mshrId = UInt(log2Up(config.mshrEntries) bits)
  val error = Bool()
}

final class MshrTable(config: OooCoreConfig = OooCoreConfig.FourIssueThreeCommit)
    extends Component {
