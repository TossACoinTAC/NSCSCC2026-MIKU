...
class OooCoreSpec extends AnyFunSuite {
  ...
  test("the initial backend locks four execution ports and three-wide retirement") {
    ...
    assert(config.enableHeadCompletionCommitBypass)
  ...
  test("the expanded-window experiment changes capacity without changing the default core") {
    ...
    assert(expandedRob.physicalRegs == 64)
    assert(expandedRob.robEntries == 64)
    assert(expandedRob.robPointerWidth == 7)
    ...
    assert(legacyGshare.predictorPhtEntriesPerBank == 1024)
