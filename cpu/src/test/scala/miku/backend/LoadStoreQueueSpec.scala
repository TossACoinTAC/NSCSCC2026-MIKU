...
class LoadStoreQueueSpec extends AnyFunSuite {
  ...
  test("four byte stores assemble one word and forward to a younger load") {
    for (banked <- Seq(false, true)) {
      val testConfig = config.copy(enableBankedLoadForwardCompletion = banked)
      ...
      SimConfig.withVerilator
        .workspacePath(
          sys.env.getOrElse("SPINAL_SIM_WORKSPACE_ROOT", "target") +
            s"/sim-workspace-ooo-lsq-byte-lane-forward-$banked"
        )
        .compile(new LoadStoreQueueProbe(testConfig))
        .doSim(s"ooo-lsq-byte-lane-forwarding-$banked", if (banked) 0x4d01 else 0x4d00) { dut =>
        dut.clockDomain.forkStimulus(period = 10)
        clearInputs(dut)
        dut.clockDomain.assertReset()
        dut.clockDomain.waitSampling(2)
        ...
        dut.io.allocate(1).isStore #= true
        dut.io.allocate(1).storeQueueIndex #= 1
        dut.io.allocate(2).robPointer #= 2
        dut.io.allocate(2).isStore #= true
        dut.io.allocate(2).storeQueueIndex #= 2
        sample(dut)
        dut.io.allocateValid #= 3
        dut.io.allocate(0).robPointer #= 3
        dut.io.allocate(0).isStore #= true
        dut.io.allocate(0).storeQueueIndex #= 3
        dut.io.allocate(1).robPointer #= 4
        dut.io.allocate(1).isStore #= false
        dut.io.allocate(1).isLoad #= true
        dut.io.allocate(1).loadQueueIndex #= 0
        sample(dut)
        dut.io.allocateValid #= 0

        for (storeIndex <- 0 until 4) {
          setStoreAgu(
            dut,
            pointer = storeIndex,
            address = 0x200 + storeIndex,
            data = storeIndex + 1,
            storeIndex = storeIndex
          )
          dut.io.agu.size #= 0
          dut.io.agu.byteMask #= 1 << storeIndex
          sample(dut)
          dut.io.aguValid #= false
        }

        setLoadAgu(dut, pointer = 4, address = 0x200, loadIndex = 0)
        sleep(1)
        assert(dut.io.aguReady.toBoolean)
        sample(dut)
        dut.io.aguValid #= false
        ...
        var forwardingWait = 0
        while (
          (!dut.io.completionValid.toBoolean || dut.io.completion.robPointer.toBigInt != 4) &&
          forwardingWait < 60
        ) {
          sample(dut)
          forwardingWait += 1
        ...
        assert(dut.io.completion.robPointer.toBigInt == 4)
        assert(dut.io.completion.pdst.toBigInt == 7)
        assert(dut.io.completion.data.toBigInt == BigInt("04030201", 16))
        assert(dut.io.loadWakeupValid.toBoolean)
        assert(dut.io.loadWakeupPdst.toBigInt == 7)
        assert(dut.io.loadWakeupRecoveryEpoch.toBigInt == 0)
        assert(!dut.io.dataRequestValid.toBoolean)
  ...
  test("a younger partial store overrides the matching byte of an older full-cover store") {
    for (banked <- Seq(false, true)) {
      val testConfig = config.copy(enableBankedLoadForwardCompletion = banked)
      ...
      SimConfig.withVerilator
        .workspacePath(
          sys.env.getOrElse("SPINAL_SIM_WORKSPACE_ROOT", "target") +
            s"/sim-workspace-ooo-lsq-full-plus-byte-forward-$banked"
        )
        .compile(new LoadStoreQueueProbe(testConfig))
        .doSim(s"ooo-lsq-full-plus-byte-forwarding-$banked", if (banked) 0x4d03 else 0x4d02) { dut =>
        dut.clockDomain.forkStimulus(period = 10)
        clearInputs(dut)
        dut.clockDomain.assertReset()
        dut.clockDomain.waitSampling(2)
        ...
        setStoreAgu(dut, pointer = 0, address = 0x200, data = BigInt("12345678", 16), storeIndex = 0)
        sample(dut)
        dut.io.aguValid #= false

        setStoreAgu(dut, pointer = 1, address = 0x200, data = BigInt("aa", 16), storeIndex = 1)
        dut.io.agu.size #= 0
        dut.io.agu.byteMask #= 1
        sample(dut)
        dut.io.aguValid #= false

        setLoadAgu(dut, pointer = 2, address = 0x200, loadIndex = 0)
        sleep(1)
        assert(dut.io.aguReady.toBoolean)
        sample(dut)
        dut.io.aguValid #= false
        ...
        var forwardingWait = 0
        while (
          (!dut.io.completionValid.toBoolean || dut.io.completion.robPointer.toBigInt != 2) &&
          forwardingWait < 60
        ) {
          sample(dut)
          forwardingWait += 1
        ...
        assert(dut.io.completionValid.toBoolean)
        assert(dut.io.completion.robPointer.toBigInt == 2)
        assert(dut.io.completion.data.toBigInt == BigInt("123456aa", 16))
        assert(!dut.io.dataRequestValid.toBoolean)
      }
    }
  }
