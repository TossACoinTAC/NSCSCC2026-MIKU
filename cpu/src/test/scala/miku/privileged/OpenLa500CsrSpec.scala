...
private final class OpenLa500CsrSimTop extends Component {
  val io = new Bundle {
    val coreClk = in Bool ()
  ...
  private val csr = new OpenLa500Csr()
...
class OpenLa500CsrSpec extends AnyFunSuite {
  test("n49 timer interrupt is level-pending and reset clears its state") {
    SimConfig.withVerilator
      ...
      .compile(new OpenLa500CsrSimTop)
