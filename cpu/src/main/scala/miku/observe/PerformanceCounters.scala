...
/** Activity-compatible implementation of `a158aa8:rtl/perf_counter.v`.
  ...
  */
final class PerformanceCounters(exposeSnapshot: Boolean = false) extends Component {
  val io = new Bundle {
    val clk = in Bool ()
    val reset = in Bool ()
...
private[miku] final class LegacyPerformanceCounters extends Component {
...
object GeneratePerformanceCounters {
    ...
    args match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "usage: GeneratePerformanceCounters [--out-dir] <output-directory>"
        )
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    config.generateVerilog(new LegacyPerformanceCounters)
  }
}
