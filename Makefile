...
SOC_CONGESTION_DCP ?= $(SOC_REPORT_DCP)
SOC_CONGESTION_OUT ?= $(BUILD_ROOT)/reports/vivado/congestion-$(shell date +%Y%m%d-%H%M%S)
...
.PHONY: help doctor status ide-setup env-build toolchain-check docs-check experiment-freeze experiment-compare timing-analyze route-analyze yosys-analyze yosys-compare test-impact perf-observation-summary \
  ...
  sim sim-prepare sim-matrix func58-sim perf20-sim linux-sim wave soc-impl soc-func soc-postroute-opt soc-report-dcp soc-congestion-review soc-archive soc-timing \
...
help:
	@printf '%s\n' \
		...
		'  make route-analyze       提取 Vivado 拥塞、overlap 与路由耗时' \
		'  make yosys-analyze       对显式冻结 RTL 做快速结构/逻辑深度分析' \
		'  make yosys-compare       比较两份同工具、同配置的 Yosys 分析' \
		...
		'  make soc-congestion-review 从 placed/physopt DCP 深入提取拥挤和控制网报告' \
...
soc-congestion-review:
	@test -f "$(SOC_CONGESTION_DCP)" || { printf 'DCP 不存在: %s\n' "$(SOC_CONGESTION_DCP)" >&2; exit 2; }
	@mkdir -p "$(SOC_CONGESTION_OUT)"
	@"$(VIVADO)" -mode batch -source "$(ROOT_DIR)/scripts/vivado/review_congestion.tcl" \
		-tclargs "$(SOC_CONGESTION_DCP)" "$(SOC_CONGESTION_OUT)"
	@printf '拥挤只读报告：%s\n' "$(SOC_CONGESTION_OUT)"
