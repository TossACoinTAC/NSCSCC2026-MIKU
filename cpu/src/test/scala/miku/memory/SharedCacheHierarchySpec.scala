...
private final class SharedCacheHierarchyProbe(config: OooCoreConfig) extends Component {
  val io = new Bundle {
    ...
    val memoryWriteDataWords = out Vec (Bits(32 bits), CacheContract.LineBytes / 4)
  ...
  val hierarchy = new SharedCacheHierarchy(config)
...
class SharedCacheHierarchySpec extends AnyFunSuite {
  private val config = OooCoreConfig.FourIssueThreeCommit

  private def sample(dut: SharedCacheHierarchyProbe): Unit = {
    dut.clockDomain.waitSampling()
    sleep(1)
  }

  private def clearInputs(dut: SharedCacheHierarchyProbe): Unit = {
  ...
  test("simultaneous L1I and L1D misses share one L2 refill without cross-routing") {
    SimConfig.withVerilator
      ...
      .compile(new SharedCacheHierarchyProbe(config))
  ...
  test("uncached stores write back and invalidate cached aliases before reaching memory") {
    SimConfig.withVerilator
      ...
      .doSim("ooo-shared-cache-uncached-store-alias", 0x4c54) { dut =>
        ...
        val beatMask = (BigInt(1) << CacheContract.BeatBits) - 1
        var backingLine = oldWord
        for (word <- 1 until CacheContract.LineBytes / 4) {
        ...
        def serviceRead(line: BigInt): BigInt = {
          ...
          for (beat <- 0 until CacheContract.BeatsPerLine) {
            ...
            dut.io.memoryReadBeat.data #=
              (line >> (beat * CacheContract.BeatBits)) & beatMask
            dut.io.memoryReadBeat.last #= beat == CacheContract.BeatsPerLine - 1
      ...
      .doSim("ooo-shared-cache-ibar-self-modifying-code", 0x4c75) { dut =>
        ...
        def serviceMemoryRead(line: BigInt): Option[BigInt] = {
          ...
          for (beat <- 0 until CacheContract.BeatsPerLine) {
            ...
            dut.io.memoryReadBeat.mshrId #= mshrId
            dut.io.memoryReadBeat.beat #= beat
            dut.io.memoryReadBeat.data #= (line >> (beat * CacheContract.BeatBits)) & beatMask
            dut.io.memoryReadBeat.last #= beat == CacheContract.BeatsPerLine - 1
            while (!dut.io.memoryReadBeatReady.toBoolean) { sample(dut) }
            sample(dut)
            if (dut.io.instructionResponseValid.toBoolean) {
