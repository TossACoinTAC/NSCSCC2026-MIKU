...
private final class OooDispatchQueueProbe(config: OooCoreConfig) extends Component {
  val io = new Bundle {
  ...
  val queue = new OooDispatchQueue(config)
...
class OooDispatchQueueSpec extends AnyFunSuite {
  private val config = OooCoreConfig.FourIssueThreeCommit

  private def clearInputs(dut: OooDispatchQueueProbe): Unit = {
  ...
  private def sample(dut: OooDispatchQueueProbe): Unit = {
    dut.clockDomain.waitSampling()
    sleep(1)
  }

  private def driveEnqueue(dut: OooDispatchQueueProbe, pointers: Seq[Int]): Unit = {
  ...
  test("rename dispatch FIFO preserves order across prefix dequeue and ring wrap") {
    SimConfig.withVerilator
      ...
      .compile(new OooDispatchQueueProbe(config))
