...
private final class OpenLa500MulSimTop extends Component {
  val io = new Bundle {
    val mulClk = in Bool ()
    val hold = in Bool ()
    val signed = in Bool ()
...
class OpenLa500MulSpec extends AnyFunSuite {
  ...
  test("each active edge captures a full signed or unsigned product and reset holds it") {
    ...
    SimConfig
      ...
      .compile(new OpenLa500MulSimTop)
  ...
  test("generated module has exactly the six native golden ports") {
    ...
    try {
      GenerateOpenLa500Mul.main(Array(outputDirectory.toString))
