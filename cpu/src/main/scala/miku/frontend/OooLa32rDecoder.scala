...
final class OooLa32rDecoder(config: OooCoreConfig = OooCoreConfig.FourIssueThreeCommit)
    extends Component {
  ...
  val io = new Bundle {
    ...
    val fetchException = in(OooExceptionMeta())
    val privilege = in Bits (2 bits)
    val interruptPending = in Bool ()
    val decoded = out(OooDecodedUop(config))
  }
  ...
  systemOperation := OooSystemOp.none
  when(instCsrRead) { systemOperation := OooSystemOp.csrRead }
  when(instCsrWrite) { systemOperation := OooSystemOp.csrWrite }
  when(instCsrXchg) { systemOperation := OooSystemOp.csrExchange }
  when(instRdCntIdW) { systemOperation := OooSystemOp.counterId }
  when(instRdCntVlW) { systemOperation := OooSystemOp.counterLow }
  when(instRdCntVhW) { systemOperation := OooSystemOp.counterHigh }
  when(instCpuCfg) { systemOperation := OooSystemOp.cpuConfig }
  when(instErtn) { systemOperation := OooSystemOp.ertn }
  when(instTlbSearch) { systemOperation := OooSystemOp.tlbSearch }
  when(instTlbRead) { systemOperation := OooSystemOp.tlbRead }
  when(instTlbWrite) { systemOperation := OooSystemOp.tlbWrite }
  when(instTlbFill) { systemOperation := OooSystemOp.tlbFill }
  when(instInvTlb) { systemOperation := OooSystemOp.invalidateTlb }
  when(instDbar) { systemOperation := OooSystemOp.dataBarrier }
  when(instIbar) { systemOperation := OooSystemOp.instructionBarrier }
  when(instIdle) { systemOperation := OooSystemOp.idle }
  when(validCacop) { systemOperation := OooSystemOp.cacheOperation }
  when(instPreload) { systemOperation := OooSystemOp.preload }
  when(instLlW) { systemOperation := OooSystemOp.loadLinked }
  when(instScW) { systemOperation := OooSystemOp.storeConditional }
  ...
  fuType := OooFuType.alu
  when(branchInstruction) { fuType := OooFuType.branch }
