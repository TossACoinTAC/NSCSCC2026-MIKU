...
private final class DispatchWindowProbe(config: OooCoreConfig) extends Component {
  val io = new Bundle {
    ...
    val output = out Vec (RenamedMicroOp(config), config.dispatchWidth)
    val outputReady = in Bits (config.dispatchWidth bits)
    val flush = in Bool ()
  }
  noIoPrefix()

  val window = new DispatchWindow(config)
...
class DispatchWindowSpec extends AnyFunSuite {
  private val config = OooCoreConfig.FourIssueThreeCommit

  private def clearInputs(dut: DispatchWindowProbe): Unit = {
    dut.io.inputValid #= 0
  ...
  private def driveInputs(dut: DispatchWindowProbe, pointers: Seq[Int]): Unit = {
  ...
  private def sample(dut: DispatchWindowProbe): Unit = {
    dut.clockDomain.waitSampling()
    sleep(1)
  }

  private def expectOutputs(dut: DispatchWindowProbe, pointers: Seq[Int]): Unit = {
  ...
  test("dispatch window compacts partial dispatch and refills every vacated slot") {
    SimConfig.withVerilator
      ...
      .compile(new DispatchWindowProbe(config))
