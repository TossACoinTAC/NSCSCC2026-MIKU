...
import miku.memory.OpenLa500DCache
...
/** The active Backend connects LACC request acceptance to dcache.addr_ok, while its read response
  ...
  */
private final class OpenLa500LaccDCacheIntegrationTop extends Component {
  val io = new Bundle {
    val coreClk = in Bool ()
  ...
  private val lacc = new OpenLa500LaccCore
  private val dcache = new OpenLa500DCache
...
class OpenLa500LaccDCacheIntegrationSpec extends AnyFunSuite {
  ...
  test("cross-line reads use addr_ok for acceptance and data_ok for response") {
    ...
    SimConfig
      ...
      .compile(new OpenLa500LaccDCacheIntegrationTop)
      .doSim("cross-line-lacc-dcache", RandomSeed) { dut =>
        ...
        def configure(count: Int, destination: BigInt): Unit = {
          dut.io.requestCommand #= OpenLa500LaccCommand.Configure
