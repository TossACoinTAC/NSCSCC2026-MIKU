...
final case class OooExceptionMeta() extends Bundle {
  val valid = Bool()
...
final case class OooDecodedUop(config: OooCoreConfig) extends Bundle {
...
final case class OooRenamedUop(config: OooCoreConfig) extends Bundle {
  val decoded = OooDecodedUop(config)
...
final case class OooIssuedUop(config: OooCoreConfig) extends Bundle {
  val renamed = OooRenamedUop(config)
...
final case class OooCommitRecord(config: OooCoreConfig) extends Bundle {
  val pc = UInt(config.xlen bits)
  val instruction = Bits(32 bits)
  val robPointer = UInt(config.robPointerWidth bits)
  val rd = UInt(config.archRegIndexWidth bits)
