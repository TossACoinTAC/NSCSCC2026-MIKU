...
private final class PerformanceCountersSimTop extends Component {
  val io = new Bundle {
    val coreClk = in Bool ()
  ...
  private val core = new PerformanceCounters(exposeSnapshot = true)
...
class PerformanceCountersSpec extends AnyFunSuite {
  test("synchronous reset, independent events, concurrency, and idle hold match the contract") {
    ...
    SimConfig
      ...
      .compile(new PerformanceCountersSimTop)
  ...
  test("generated legacy wrapper has exactly the locked nine input ports and named counters") {
    ...
    try {
      GeneratePerformanceCounters.main(Array(outputDirectory.toString))
