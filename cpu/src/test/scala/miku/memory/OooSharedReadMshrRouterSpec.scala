...
private final class OooSharedReadMshrRouterProbe(config: OooCoreConfig) extends Component {
  val io = new Bundle {
  ...
  val router = new OooSharedReadMshrRouter(config)
...
class OooSharedReadMshrRouterSpec extends AnyFunSuite {
  private val config = OooCoreConfig.FourIssueThreeCommit

  private def sample(dut: OooSharedReadMshrRouterProbe): Unit = {
    dut.clockDomain.waitSampling()
    sleep(1)
  }

  private def clearInputs(dut: OooSharedReadMshrRouterProbe): Unit = {
