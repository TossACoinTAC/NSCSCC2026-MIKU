...
private final class LaccCoreSimTop extends Component {
  val io = new Bundle {
    val coreClk = in Bool ()
...
class LaccCoreSpec extends AnyFunSuite {
    ...
    SimConfig
      ...
      .doSim("lacc-cycle-contract", RandomSeed) { dut =>
        ...
        def configure(count: Int, destination: BigInt): Unit = {
          driveRequest(LaccCommand.Configure, count, destination)
        ...
        driveRequest(LaccCommand.Lmadd, BigInt("1000", 16), BigInt("2000", 16))
        ...
        driveRequest(LaccCommand.Lmadd, BigInt("4000", 16), BigInt("5000", 16))
        coreRisingEdge()
        ...
        driveRequest(LaccCommand.Lmadd, BigInt("8000", 16), BigInt("9000", 16))
        coreRisingEdge()
        ...
        driveRequest(LaccCommand.Lmadd, BigInt("b000", 16), BigInt("c000", 16))
        coreRisingEdge()
  ...
  test("generated legacy wrapper has exactly the golden LACC ports") {
    ...
    try {
      GenerateLaccCore.main(Array(outputDirectory.toString))
