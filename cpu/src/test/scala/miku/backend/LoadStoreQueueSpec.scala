...
class LoadStoreQueueSpec extends AnyFunSuite {
  ...
  test("cached store requests turn over without an empty buffer cycle") {
    SimConfig.withVerilator
      .workspacePath(
        sys.env.getOrElse("SPINAL_SIM_WORKSPACE_ROOT", "target") +
          "/sim-workspace-ooo-lsq-store-turnover"
      )
      .compile(new LoadStoreQueueProbe(config))
      .doSim("ooo-lsq-store-turnover", 0x4c81) { dut =>
        dut.clockDomain.forkStimulus(period = 10)
        clearInputs(dut)
        dut.io.translationBypassEligible #= true
        dut.clockDomain.assertReset()
        ...
        for (lane <- 0 until 2) {
          ...
          dut.io.allocate(lane).storeQueueIndex #= lane
        }
        sample(dut)
        dut.io.allocateValid #= 0

        dut.io.translationBypassPhysicalAddress #= 0x80000100L
        setStoreAgu(dut, 0, 0x100, BigInt("11111111", 16), storeIndex = 0)
        sample(dut)
        dut.io.aguValid #= false

        var firstCompletionWait = 0
        while (!dut.io.completionValid.toBoolean && firstCompletionWait < 8) {
          sample(dut)
          firstCompletionWait += 1
        }
        assert(dut.io.completionValid.toBoolean)
        assert(dut.io.completion.robPointer.toBigInt == 0)

        dut.io.translationBypassPhysicalAddress #= 0x80000200L
        setStoreAgu(dut, 1, 0x200, BigInt("22222222", 16), storeIndex = 1)
        sample(dut)
        dut.io.aguValid #= false
        ...
        var secondCompletionWait = 0
        while (!dut.io.completionValid.toBoolean && secondCompletionWait < 8) {
          sample(dut)
          secondCompletionWait += 1
        }
        assert(dut.io.completionValid.toBoolean)
        assert(dut.io.completion.robPointer.toBigInt == 1)

        dut.io.commitValid #= 1
        dut.io.commit(0).robPointer #= 1
        dut.io.commit(0).isStore #= true
        dut.io.commit(0).storeQueueIndex #= 1
        sample(dut)
        dut.io.commitValid #= 0
        assert(dut.io.dataRequestValid.toBoolean)
        assert(dut.io.dataRequest.robPointer.toBigInt == 0)
