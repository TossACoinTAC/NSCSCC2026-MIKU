...
final case class OooTlbProbeRequest() extends Bundle {
  val vppn = Bits(19 bits)
  val oddPage = Bool()
  val asid = Bits(10 bits)
}

final case class OooTlbLookupResult() extends Bundle {
  val found = Bool()
...
final case class OooTlbEntryState() extends Bundle {
  val vppn = Bits(19 bits)
  val asid = Bits(10 bits)
  val global = Bool()
  val pageSize = Bits(6 bits)
...
/** Two four-entry micro-TLBs backed by one four-entry-per-cycle main-TLB walker.
  ...
  */
final class OooHierarchicalTlb(microEntries: Int = 4, walkEntriesPerCycle: Int = 4)
    extends Component {
  ...
  val io = new Bundle {
    ...
    val instructionRequest = slave(Stream(OooTlbProbeRequest()))
    val instructionResponse = master(Flow(OooTlbLookupResult()))
    val dataRequest = slave(Stream(OooTlbProbeRequest()))
    val dataResponse = master(Flow(OooTlbLookupResult()))

    val writeValid = in Bool ()
    val writeIndex = in UInt (5 bits)
    val writeEntry = in(OooTlbEntryState())
    ...
    val readEntry = out(OooTlbEntryState())
  ...
  val area = new ClockingArea(domain) {
    ...
    val entryBanks = Vec.fill(walkEntriesPerCycle)(Vec.fill(8)(Reg(OooTlbEntryState())))

    private def entryAt(index: Int): OooTlbEntryState =
      entryBanks(index % walkEntriesPerCycle)(index / walkEntriesPerCycle)

    private def pageMatches(entry: OooTlbEntryState, vppn: Bits): Bool =
      Mux(
    ...
    private def entryMatches(entry: OooTlbEntryState, vppn: Bits, asid: Bits): Bool =
    ...
    val instructionMicro = Vec.fill(microEntries)(Reg(OooTlbEntryState()))
    ...
    val dataMicro = Vec.fill(microEntries)(Reg(OooTlbEntryState()))
    ...
    val walkEntries = Vec(OooTlbEntryState(), walkEntriesPerCycle)
