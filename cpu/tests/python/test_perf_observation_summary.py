...
def write_observation_matrix(root: Path) -> Path:
    ...
    with matrix.open("w", newline="", encoding="utf-8") as stream:
        ...
        for benchmark in PERF20_BENCHMARKS:
            ...
            counters["store_data"] = {
                "multiple_ready_cycles": 20,
                "out_of_age_order_cycles": 5,
            }
            counters["predictor_history"] = {
                "groups": 100,
                "conditional_steps": 125,
                "multi_conditional_groups": 25,
            }
            counters["l1d_response_arbitration"] = {
                "lookup_hit_load_cycles": 200,
                "miss_waiter_ready_cycles": 50,
                "hit_waiter_collision_cycles": 10,
...
class PerfObservationSummaryTest(unittest.TestCase):
    def test_summarizes_identity_bound_perf20_roi(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertAlmostEqual(
                result["summary"]["derived"]["store_data_out_of_age_per_multiple_ready"],
                0.25,
            )
            self.assertEqual(raw["predictor_history_groups"], 2000)
            self.assertEqual(raw["predictor_history_conditional_steps"], 2500)
            self.assertAlmostEqual(
                result["summary"]["derived"]["predictor_history_multi_group_ratio"],
                0.25,
            )
            self.assertEqual(
    ...
    def test_summarizes_existing_v8_matrix_without_store_data_fields(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            matrix = write_observation_matrix(Path(directory))
            for path in Path(directory).glob("perf20__*/seed_0/limit_1ns/m01-counters.json"):
                counters = json.loads(path.read_text(encoding="utf-8"))
                counters["schema_version"] = "miku-perf-observation-v8"
                counters.pop("store_data")
                counters.pop("predictor_history")
                counters.pop("l1d_response_arbitration")
                path.write_text(json.dumps(counters), encoding="utf-8")
                manifest = path.with_name("run-manifest.txt")
                lines = manifest.read_text(encoding="utf-8").splitlines()
                counter_hash = hashlib.sha256(path.read_bytes()).hexdigest()
            ...
            self.assertEqual(
                result["summary"]["raw"]["store_data_multiple_ready_cycles"], 0
            )

    def test_summarizes_existing_v9_matrix_without_v10_fields(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            matrix = write_observation_matrix(Path(directory))
            for path in Path(directory).glob("perf20__*/seed_0/limit_1ns/m01-counters.json"):
                counters = json.loads(path.read_text(encoding="utf-8"))
