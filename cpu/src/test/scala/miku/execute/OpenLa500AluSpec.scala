...
private final class OpenLa500AluSimTop extends Component {
  val io = new Bundle {
    val aluOp = in Bits (OpenLa500AluOp.Width bits)
...
class OpenLa500AluSpec extends AnyFunSuite {
  ...
  private def withSimulation(
      ...
      testBody: (OpenLa500AluSimTop, (Int, BigInt, BigInt) => Unit) => Unit
  ): Unit = {
    val workspaceRoot =
    ...
    SimConfig
      ...
      .compile(new OpenLa500AluSimTop)
  ...
  test("generated module has only the native golden ports") {
    ...
    try {
      GenerateOpenLa500Alu.main(Array(outputDirectory.toString))
