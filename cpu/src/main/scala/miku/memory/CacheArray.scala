...
final class CacheArray(
  ...
  val io = new Bundle {
    ...
    val hitData = out Bits (CacheContract.LineBits bits)
    val wayData = out Vec (Bits(CacheContract.LineBits bits), geometry.ways)
    ...
    val victimData = out Bits (CacheContract.LineBits bits)
    ...
    val writeData = in Bits (CacheContract.LineBits bits)
    ...
    val maintenanceEntryData = out Bits (CacheContract.LineBits bits)
  ...
  val dataMemories = Array.fill(geometry.ways)(
    Mem(Bits(CacheContract.LineBits bits), geometry.sets)
  )
  ...
  val dataRead = Vec(Bits(CacheContract.LineBits bits), geometry.ways)
